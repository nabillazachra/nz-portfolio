...
const Button = ({ children, onClick, variant = 'primary', className = '', type = 'button' }: any) => {
  ...
  const variants = {
    ...
    outline: "border-2 border-slate-200 hover:border-primary hover:text-primary hover:bg-blue-50/50"
  };
  return (
    <motion.button 
...
const Card = ({ children, className = '', noHover = false }: any) => (
  <motion.div 
    ...
    className={`bg-white rounded-2xl shadow-sm border border-slate-100 p-6 transition-all duration-300 ${className}`}
  >
    {children}
  </motion.div>
);
...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/80 backdrop-blur-md shadow-sm border-b border-slate-100 py-2' : 'bg-transparent py-4'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <Link to="/" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="text-2xl font-serif font-bold text-slate-900 cursor-pointer flex items-center gap-1 group">
            ...
            <div className="px-4 pt-2 pb-6 space-y-2">
              {navLinks.map((link) => (
                <button
                  key={link.name}
                  onClick={() => handleNavClick(link.id)}
                  className="block w-full text-left px-3 py-3 text-base font-medium text-slate-600 hover:text-primary hover:bg-blue-50 rounded-lg transition-colors"
                >
                  {link.name}
                </button>
              ))}
...
const Home = () => {
  return (
    <div className="pt-20 lg:pt-28 pb-20 overflow-hidden">
      {/* Decorative Blobs */}
      <div className="absolute top-0 right-0 -z-10 w-[600px] h-[600px] bg-blue-100/50 rounded-full blur-[100px] animate-blob mix-blend-multiply opacity-70"></div>
      <div className="absolute top-0 left-0 -z-10 w-[600px] h-[600px] bg-purple-100/50 rounded-full blur-[100px] animate-blob animation-delay-2000 mix-blend-multiply opacity-70"></div>
      <div className="absolute -bottom-8 left-20 -z-10 w-[600px] h-[600px] bg-pink-100/50 rounded-full blur-[100px] animate-blob animation-delay-4000 mix-blend-multiply opacity-70"></div>
      ...
      <section className="relative py-10 lg:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white border border-slate-200 shadow-sm text-primary text-xs font-semibold tracking-wide uppercase mb-6">
            ...
            >
              ...
              >
                ...
                <img 
                  ...
                  alt="Nabilla Zachra" 
                  className="relative w-full h-full object-cover rounded-[2rem] border-8 border-white shadow-2xl rotate-3 hover:rotate-0 transition-all duration-500"
                />
                ...
                <motion.div 
                  ...
                  transition={{ delay: 1 }}
                  className="absolute -bottom-6 -left-6 bg-white p-4 rounded-xl shadow-xl flex items-center gap-3 border border-slate-50 z-20"
                >
                ...
                <motion.div 
                  ...
                  className="absolute top-10 -right-10 bg-white p-4 rounded-xl shadow-xl border border-slate-50 z-20"
                >
                  ...
                  <p className="text-xs text-slate-500 font-medium mt-2 text-center">50+ Happy Clients</p>
                </motion.div>
              </motion.div>
            </motion.div>
          </div>
          ...
          >
            {SKILLS.slice(0, 4).map((skill) => (
              <motion.div key={skill.name} variants={itemScale}>
                <Card className="text-center h-full hover:bg-blue-50/50 group">
                  <div className="w-14 h-14 bg-blue-50 rounded-2xl flex items-center justify-center text-primary mx-auto mb-4 group-hover:scale-110 group-hover:bg-primary group-hover:text-white transition-all duration-300">
                    <CheckCircle size={28} />
                  </div>
          ...
          <div className="space-y-8">
            {WORK_EXPERIENCE.map((job, idx) => (
              <motion.div 
                ...
                className="relative pl-8 border-l-2 border-slate-200 hover:border-primary transition-colors duration-300 group"
              >
                <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-white border-2 border-slate-300 group-hover:border-primary group-hover:scale-125 transition-all duration-300"></div>
                <h4 className="text-xl font-bold text-slate-900 group-hover:text-primary transition-colors">{job.role}</h4>
                <div className="flex flex-wrap gap-2 text-sm text-slate-500 mb-3 mt-1">
            ...
            {EDUCATION.map((edu, idx) => (
              ...
              >
                <Card className="h-full border-t-4 border-t-transparent hover:border-t-primary">
                  <h4 className="text-lg font-bold text-slate-900">{edu.company}</h4>
                  <p className="text-primary font-medium mb-1">{edu.role}</p>
                  <p className="text-sm text-slate-400 mb-4">{edu.period}</p>
                  <p className="text-sm text-slate-600">{edu.description[0]}</p>
                </Card>
              </motion.div>
            ))}
          </div>
...
const Portfolio = () => {
  ...
  return (
    <section className="py-24 bg-white relative overflow-hidden">
      {/* Background Blobs */}
      <div className="absolute top-1/2 right-0 -translate-y-1/2 w-96 h-96 bg-blue-50/80 rounded-full blur-3xl -z-10"></div>
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-purple-50/80 rounded-full blur-3xl -z-10"></div>
          ...
          >
            {projects.map((project) => (
              <motion.div key={project.id} variants={itemScale}>
                <div className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-blue-900/10 transition-all duration-500 border border-slate-100 flex flex-col h-full hover:-translate-y-2">
                  <div className="relative h-60 overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-start p-6">
                      <span className="text-white font-medium flex items-center gap-2 translate-y-4 group-hover:translate-y-0 transition-transform duration-300">View Details <ExternalLink size={16} /></span>
                    </div>
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <p className="text-slate-600 text-sm mb-6 line-clamp-3 flex-1 leading-relaxed">{project.description}</p>
                    <div className="text-xs font-medium text-slate-400 pt-4 border-t border-slate-100 flex justify-between items-center">
                      <span>{project.date}</span>
                      <span className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-colors">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          >
            <Card className="text-center hover:border-primary/30 group">
              <div className="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center text-primary mx-auto mb-4 group-hover:bg-primary group-hover:text-white transition-all">
                <Mail size={22} />
              </div>
              <h4 className="font-bold text-slate-900 mb-1">Email</h4>
              <p className="text-sm text-slate-500 break-all">{CONTACT_INFO.email}</p>
            </Card>
            <Card className="text-center hover:border-primary/30 group">
              <div className="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center text-primary mx-auto mb-4 group-hover:bg-primary group-hover:text-white transition-all">
                <Phone size={22} />
              ...
              <p className="text-sm text-slate-500">{CONTACT_INFO.phone}</p>
            </Card>
            <Card className="text-center hover:border-primary/30 group">
              <div className="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center text-primary mx-auto mb-4 group-hover:bg-primary group-hover:text-white transition-all">
                <Linkedin size={22} />
              </div>
              <h4 className="font-bold text-slate-900 mb-1">Social</h4>
              <p className="text-sm text-slate-500">{CONTACT_INFO.linkedin}</p>
            </Card>
          </motion.div>
      ...
      >
        <Card>
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-serif font-bold text-slate-900">Admin Login</h2>
...
const Dashboard = () => {
  ...
  return (
    ...
    >
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Welcome back, Nabilla!</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="flex items-center gap-4 hover:shadow-lg transition-shadow border-l-4 border-l-blue-500" noHover>
          <div className="p-4 bg-blue-100 text-primary rounded-xl"><User size={24} /></div>
          <div>
        ...
        <Card className="flex items-center gap-4 hover:shadow-lg transition-shadow border-l-4 border-l-purple-500" noHover>
          <div className="p-4 bg-purple-100 text-purple-600 rounded-xl"><FolderPlus size={24} /></div>
          <div>
        ...
        <Card className="flex items-center gap-4 hover:shadow-lg transition-shadow border-l-4 border-l-green-500" noHover>
          <div className="p-4 bg-green-100 text-green-600 rounded-xl"><Mail size={24} /></div>
          <div>
      ...
      <Card className="h-[450px]" noHover>
        <h3 className="text-lg font-bold text-slate-800 mb-6">Visitor Analytics</h3>
    ...
    >
      ...
      <AnimatePresence>
        {isFormOpen && (
          ...
          >
            <Card className="mb-8 border-primary/20 ring-4 ring-primary/5" noHover>
        ...
        {projects.map(project => (
          <Card key={project.id} className="relative group overflow-hidden" noHover>
            ...
            <div className="flex justify-between items-center mt-2">
               <span className="text-sm text-primary font-medium bg-blue-50 px-2 py-0.5 rounded">{project.category}</span>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                <th className="p-5 font-semibold text-slate-700">From</th>
                <th className="p-5 font-semibold text-slate-700">Subject</th>
              ...
              {messages.map(msg => (
                <tr key={msg.id} className="hover:bg-slate-50 transition-colors group cursor-pointer">
                  ...
                  <td className="p-5">
                    ...
                    <div className="text-slate-500 truncate max-w-xs mt-0.5 opacity-80 group-hover:opacity-100">{msg.message}</div>
                  </td>
                  <td className="p-5 text-slate-500">{msg.date}</td>
                  <td className="p-5">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${msg.read ? 'bg-slate-100 text-slate-600 border-slate-200' : 'bg-green-50 text-green-600 border-green-200'}`}>
                      {msg.read ? 'Read' : 'New'}
                    </span>
                  </td>
                </tr>
